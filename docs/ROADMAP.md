...
6. **Probador virtual 3D** (✅ funcional en este repo) — `/probador?producto=<id>`, maniquí Three.js escalado con la medición corporal guardada de la usuaria (`BodyProfile`, reaprovecha `/medidas`), botón "Ver en 3D" en cada producto del catálogo. La prenda se arma según el tipo inferido del producto (`inferGarmentKind()`): blusa (cintura hacia arriba), falda (cadera hacia abajo), chaqueta (capa sobre una blusa base) o conjunto (torso + falda juntos), con el color real del producto elegido (`colorNameToHex()`). Rotación 360° real vía `OrbitControls` (arrastrar/pellizcar), no solo 3 vistas fijas. Sigue siendo geometría primitiva (cilindros/cápsulas), no un escaneo 3D del cuerpo ni simulación física de tela — ver detalle en ARCHITECTURE.md.
6b. **Probador con cámara en vivo** (✅ funcional en este repo) — `/probador/camara/[id]`, botón "Probar con mi cámara" en cada producto del catálogo. Usa la cámara (PC o celular) + MediaPipe Pose Landmarker en modo VIDEO (100% en el navegador, mismo modelo que la medición corporal) para rastrear hombros/cadera en cada cuadro y superponer la foto real del producto ajustada a esa posición/tamaño/rotación. Es un ajuste de posición en tiempo real sobre un rectángulo semitransparente, no un recorte de silueta de la prenda ni simulación física de tela — eso requeriría un motor de AR de moda dedicado (ej. Zeekit/Vue.ai), fuera de alcance sin esa integración pagada.
